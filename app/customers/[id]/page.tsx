...
export default function CustomerDetailPage() {
  ...
  const { totalSpent, totalPaid, totalDue } = useMemo(() => {
    let totalSpent = 0;
    let totalPaid = 0;
    let totalDue = 0;
    ...
    for (const sale of sales ?? []) {
      ...
      totalPaid += sale.advancePaid;
      totalDue += sale.balance;
    }

    return { totalSpent, totalPaid, totalDue };
  }, [sales]);
                    ...
                    {(["re", "le"] as const).map((eye) => {
                      ...
                      return (
                        <div key={eye} className="space-y-2">
                          <h4 className="font-semibold text-sm border-b pb-1 uppercase">
                          ...
                          {(["dv", "nv"] as const).map((dist) => (
                            <div
                              key={dist}
                              className="grid grid-cols-5 gap-1 text-center items-center"
                            >
                              ...
                              {["sph", "cyl", "axis", "va"].map((f) => (
                                <div
                                  key={f}
                                  className="bg-muted rounded py-1 px-0.5 text-xs"
                                >
                          ...
                          <div className="text-right">
                            ...
                            {sale.balance > 0 && (
                              <p className="text-xs text-red-500">
                                Due: ₹{sale.balance.toLocaleString("en-IN")}
                              </p>
                            )}
                          </div>
                          {/* Sale action menu */}
                        ...
                        <div className="space-y-1 border-t pt-2">
                          {sale.payments.map((p) => (
                            ...
                            >
                              ...
                              <span>
                                {safeFormatDate(p.date, "dd MMM, hh:mm a")}
                              </span>
                            </div>
                          ))}
                        </div>
      ...
      <Dialog open={payDialogOpen} onOpenChange={setPayDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedSale && safeFormatDate(selectedSale.date, "dd MMM yyyy")}{" "}
      ...
      <Dialog open={editCustomerOpen} onOpenChange={setEditCustomerOpen}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Update customer profile and latest eye prescription.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditCustomer((s) => ({ ...s, name: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditCustomer((s) => ({ ...s, phone: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-1.5 col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditCustomer((s) => ({ ...s, email: e.target.value }))
                  }
                />
              </div>
            </div>
            ...
            <div className="pt-2 border-t mt-4">
              ...
              <p className="text-xs text-muted-foreground">
                To update the prescription values, please{" "}
                <Link
                  href={`/add-sale?customerId=${customer.id}`}
                  className="text-primary hover:underline"
                  onClick={() => setEditCustomerOpen(false)}
                >
                  add a new sale
                </Link>
                . This ensures proper visit history tracking.
              </p>
            </div>
          </div>
          <div className="flex justify-end gap-3">
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              all their <strong>{sales?.length ?? 0} sale(s)</strong>. This
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={editSaleOpen} onOpenChange={setEditSaleOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Update sale details and amounts.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEditSale((s) => ({ ...s, totalAmount: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-1.5">
                ...
                <Input
                  type="number"
                  value={editSale.advancePaid}
                  onChange={(e) =>
                    setEditSale((s) => ({ ...s, advancePaid: e.target.value }))
                  }
                />
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setEditSale((s) => ({ ...s, notes: e.target.value }))
                }
              />
            </div>
          </div>
      ...
      <Dialog open={!!deleteSaleId} onOpenChange={() => setDeleteSaleId(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              This will permanently delete this sale record and its payment
